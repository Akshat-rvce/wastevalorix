...
const MarketplacePage = () => {
  ...
  const filterFromResults = location.state?.filterFromResults;
  ...
  if (filterFromResults && resultData?.pricing) {
    const userMin = resultData.pricing.yourPriceMin;
    filteredListings.sort((a, b) => {
      const aMatches = a.priceRangeMax >= userMin;
      const bMatches = b.priceRangeMax >= userMin;
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-6 py-8">
      ...
      {filterFromResults && resultData && (
        <div className="bg-accent/10 border border-accent rounded-xl p-4 mb-8 flex items-center gap-4">
          <div className="text-3xl">🎯</div>
          <div>
            <h3 className="text-accent font-bold">Filtered for your {resultData.wasteType}</h3>
            <p className="text-sm text-text-primary">Showing buyers interested in materials matching your AI quality assessment.</p>
          </div>
        </div>
      )}
          ...
          {filteredListings.map((listing, idx) => (
            ...
            >
              ...
              <div className="p-6 flex-1 flex flex-col">
                ...
                {filterFromResults && resultData?.pricing && (
                  <div className="mb-4">
                    {listing.priceRangeMax >= resultData.pricing.yourPriceMin ? (
                      <span className="inline-block bg-[#00E676]/20 text-[#00E676] text-xs px-2 py-1 rounded font-bold border border-[#00E676]/30">
                        {listing.priceRangeMax > resultData.pricing.yourPriceMax ? "Above your estimate ⭐" : "Matches your estimate ✓"}
                      </span>
                    ) : (
                      <span className="inline-block bg-[#FF5252]/10 text-[#FF5252] text-xs px-2 py-1 rounded font-bold border border-[#FF5252]/20">
                        Below your estimate
                      </span>
                    )}
                  </div>
